                                        ...
                                        <>
                                            {filtered.map(p => (
                                                <div
                                                    key={p}
                                                    onClick={() => {
                                                        setFormData(prev => ({ ...prev, role: p }));
                                                        setIsProfessionsOpen(false);
                                                        setProfessionSearch('');
                                                    }}
                                                    className="px-4 py-2.5 hover:bg-primary/5 cursor-pointer text-sm font-medium"
                                                >
                                                    {p}
                                                </div>
                                            ))}
                                            {/* Show "Other" option when search is non-empty and has no or few results */}
                                            {professionSearch.trim().length > 0 && (
                                                <div
                                                    onClick={() => {
                                                        if (professionSearch.trim()) {
                                                            setFormData(prev => ({ ...prev, role: professionSearch.trim() }));
                                                            setIsProfessionsOpen(false);
                                                            setProfessionSearch('');
                                                        }
                                                    }}
                                                    className="px-4 py-3 border-t border-slate-100 hover:bg-amber-50 cursor-pointer text-sm font-semibold text-amber-700 flex items-center gap-2"
                                                >
                                                    <span className="material-symbols-outlined text-base">edit</span>
                                                    {filtered.length === 0
                                                        ? `No results — set "${professionSearch}" as custom profession`
                                                        : `Use "${professionSearch}" as custom profession`
                                                    }
                                                </div>
                                            )}
                                        </>
