                ...
                deleteMany: async (query) => {
                    ...
                    for (const [key, doc] of store.entries()) {
                        ...
                        for (const qKey in query) {
                            const filterVal = query[qKey];
                            if (filterVal && typeof filterVal === 'object' && !Array.isArray(filterVal)) {
                                if ('$lt' in filterVal && !(doc[qKey] < filterVal.$lt)) match = false;
                                if ('$gte' in filterVal && !(doc[qKey] >= filterVal.$gte)) match = false;
                            } else {
                                if (doc[qKey] !== filterVal) match = false;
                            }
                        }
                        if (match) {
                            store.delete(key);
...
if (!uri || (!uri.startsWith('mongodb://') && !uri.startsWith('mongodb+srv://'))) {
    console.warn("⚠️ MONGODB_URI is not set or is invalid. Falling back to Mock In-Memory Database.");
    mockClient.connectionError = "MONGODB_URI environment variable is not defined or is invalid.";
